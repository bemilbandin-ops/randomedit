...
interface ShortcutDefinition extends ShortcutCommand {
  premiereMac?: string;
  resolveMac?: string;
}
...
export const shortcutCommands: ShortcutDefinition[] = [
  ...
  { id: 'addEdit', name: 'Add Edit / Split Clip', category: 'Editing', description: 'Split the selected clip at the playhead.', premiere: 'Ctrl+K', premiereMac: 'Cmd+K', resolve: 'Ctrl+B', resolveMac: 'Cmd+B' },
...
export function getPresetBindings(profile: Exclude<ShortcutProfile, 'custom'>, isMac: boolean): ShortcutBindings {
  return Object.fromEntries(
    shortcutCommands.map((command) => {
      if (profile === 'premiere') {
        return [command.id, isMac && command.premiereMac ? command.premiereMac : command.premiere];
      }
      return [command.id, isMac && command.resolveMac ? command.resolveMac : command.resolve];
    }),
  );
}
