...
export interface ShortcutDefinition extends ShortcutCommand {
  premiereMac?: string;
  resolveMac?: string;
  premiereMacDisplay?: string;
  resolveMacDisplay?: string;
}
...
export const shortcutCommands: ShortcutDefinition[] = [
  ...
  { id: 'razorTool', name: 'Razor / Blade Edit Mode', category: 'Tools', description: 'Choose the cutting tool used to split clips.', premiere: 'C', resolve: 'B' },
  {
    ...
    name: 'Ripple Delete',
    category: 'Editing',
    description: 'Remove the selected clip and close the gap.',
    premiere: 'Shift+Delete',
    premiereMac: 'Shift+Delete',
    premiereMacDisplay: 'Shift+Forward Delete',
    resolve: 'Shift+Backspace',
    resolveMac: 'Shift+Delete',
  },
...
export function referenceDisplayBinding(
  commandId: string,
  profile: Exclude<ShortcutProfile, 'custom'>,
  isMac: boolean,
): string {
  const command = shortcutCommands.find((item) => item.id === commandId);
  if (!command) return '';
  if (isMac && profile === 'premiere' && command.premiereMacDisplay) return command.premiereMacDisplay;
  if (isMac && profile === 'resolve' && command.resolveMacDisplay) return command.resolveMacDisplay;
  return referenceBinding(commandId, profile, isMac);
}
